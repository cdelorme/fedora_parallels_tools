...
static void tg_submit(TG_PAGED_REQUEST *dst, struct pci_dev *pdev)
{
	unsigned long flags;
...
static int prl_tg_initialize(struct pci_dev *pdev, struct tg_dev *dev)
{
	...
	rc = request_irq(dev->irq, prl_tg_interrupt, IRQF_SHARED, board_info[dev->board].nick, dev);
	if (rc) {
