...
//#define NET_VIRTIO_NOTIFY_ON_EMPTY		()
#define NET_VIRTIO_RX_DIRECT			(1 << 8)
#define NET_VIRTIO_TX_DIRECT			(1 << 9)
#define NET_VIRTIO_RX_OFFLOAD			(1 << 10)
#define NET_VIRTIO_TX_OFFLOAD			(1 << 11)
	UINT	uVirtIoFlags;
	// Virtio rx irq request
	UINT	bVirtIoRxIrqReq;
	// Virtio tx irq request
	UINT	bVirtIoTxIrqReq;
	// Direct rx mode (virtio only):
	// Monitor lock guest buffer, put list of locked pages to aBuffer and
	// advance uTail.
	// Pvsnet receive packet, copy it to guest's memory and advance uHead.
	// Monitor unlock quest pages and advance uVirtIoHead.
	// uVirtIoHead <= uHead <= uTail
	UINT	uVirtIoHead;
